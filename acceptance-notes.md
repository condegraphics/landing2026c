...
La solución propia reemplaza completamente el flujo temporal anterior. El formulario publica sus campos mediante una solicitud asíncrona a `contact.php` en Hostinger, mantiene validación HTML, incluye protección honeypot, valida nuevamente en servidor y muestra una confirmación breve dentro de la misma landing. El endpoint usa PHPMailer con SMTP Relay de Google Workspace mediante TLS y autorización por la IP de Hostinger, y prepara un correo multipart `text/plain` + `text/html`, con tablas y estilos inline en la paleta azul institucional, para que la notificación sea legible en clientes de correo con y sin soporte HTML. El remitente queda fijado en `info@condegraphics.com` y el `Reply-To` utiliza el email del potencial cliente. No se guardan credenciales; la entrega requiere que la regla de relay, la IP autorizada y SPF, DKIM y DMARC permanezcan coherentes.

La experiencia posterior al envío fue revisada según buenas prácticas de feedback accesible: la ruta normal muestra un mensaje inline con `role="status"` y `aria-live="polite"`, sin sacar al usuario de la landing ni utilizar un popup bloqueante. El mensaje confirma la recepción, comunica el análisis del negocio, presencia digital y objetivos, y establece una primera devolución en 48 horas hábiles.
