...
La solución propia reemplaza completamente el flujo temporal anterior. El formulario publica sus campos mediante POST a `contact.php` en Hostinger, mantiene validación HTML, incluye protección honeypot, valida nuevamente en servidor y redirige por HTTP 303 a `gracias.html`. El endpoint usa PHPMailer con SMTP autenticado y prepara un correo multipart `text/plain` + `text/html`, con tablas y estilos inline en la paleta azul institucional, para que la notificación sea legible en clientes de correo con y sin soporte HTML. El remitente queda fijado en `info@condegraphics.com` y el `Reply-To` utiliza el email del potencial cliente. La contraseña de aplicación se configura sólo en el archivo privado `contact-config.php`, excluido de Git; la entrega requiere validar la cuenta SMTP, SPF, DKIM y DMARC en Google Workspace/Hostinger.
