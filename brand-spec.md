...
En desktop, el bloque de contacto se distribuye en dos columnas 40/60: propuesta y texto a la izquierda, formulario a la derecha. Los valores del bloque de contacto alternativo usan una escala menor, `overflow-wrap` y un interlineado controlado para evitar choques entre email, teléfono y ubicación. La dirección visible es `Olazábal 2715 | C1428DHJ · Buenos Aires, Argentina` y está enlazada a Google Maps sin subrayado.

El favicon se genera desde el isotipo circular azul del logo oficial y se publica en `img/favicon/favicon.svg`, acompañado por `site.webmanifest` para navegadores compatibles.
